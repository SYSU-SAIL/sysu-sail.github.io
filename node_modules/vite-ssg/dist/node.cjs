...
const state = require('./shared/vite-ssg.bee8a5a9.cjs');
...
async function build(ssgOptions = {}, viteConfig = {}) {
  ...
  const ssgOutTempFolder = node_path.join(root, ".vite-ssg-temp");
  const ssgOut = node_path.join(ssgOutTempFolder, Math.random().toString(36).substring(2, 12));
  ...
  if (fs__default.existsSync(ssgOutTempFolder))
    await fs__default.remove(ssgOutTempFolder);
